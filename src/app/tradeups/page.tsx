        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:min-w-[360px]">
            <Metric label="Total cost">
              <span className="text-white"><ScrambleText text={formatPrice(entry.total_cost_cents)} scrambleSpeed={50}delay={400}/></span>
            </Metric>
            <Metric label="Estimated return">
              <span className="text-white"><ScrambleText text={formatPrice(entry.expected_return_cents)} scrambleSpeed={50}delay={400}/></span>
            </Metric>
            <Metric label="Estimated value">
              <span className={evPositive ? 'text-green-400' : 'text-red-400'}>
                {evPositive ? '+' : ''}{<ScrambleText text={formatPrice(entry.ev_cents)} scrambleSpeed={50}delay={400}/>}
              </span>
            </Metric>
            <Metric label="Estimated ROI">
              <span className={evPositive ? 'text-green-400' : 'text-red-400'}>
                {entry.roi_percent > 0 ? '+' : ''}{<ScrambleText text={entry.roi_percent.toFixed(2)} scrambleSpeed={50}delay={400}/>}%
              </span>
            </Metric>
          </div>
        </div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4">
            ...
            <div className="mt-3 text-sm text-zinc-400">
              <div className="flex items-center justify-between gap-3">
                <span>Per input</span>
                <span className="text-zinc-300"><ScrambleText text={formatPrice(entry.input.price_cents)} scrambleSpeed={50} delay={400}/></span>
              </div>
              <div className="mt-2 flex items-center justify-between gap-3">
                <span>Wear</span>
                <span className="text-zinc-300"><ScrambleText text={entry.input.wear_name} scrambleSpeed={50} delay={400}/></span>
              </div>
              <div className="mt-2 flex items-center justify-between gap-3">
                <span>Estimated avg float</span>
                <span className="text-zinc-300"><ScrambleText text={entry.input.expected_float.toFixed(5)} scrambleSpeed={50} delay={400}/></span>
              </div>
              <div className="mt-2 flex items-center justify-between gap-3">
                <span>Contract size</span>
                <span className="text-zinc-300"><ScrambleText text={`${entry.num_inputs} inputs`} scrambleSpeed={50} delay={400}/></span>
              </div>
              <div className="mt-2 flex items-center justify-between gap-3">
                <span>Best priced outcome</span>
                <span className="text-zinc-300"><ScrambleText text={formatPrice(entry.best_outcome_price_cents)} scrambleSpeed={50} delay={400}/></span>
              </div>
            </div>
          </div>
        </div>
